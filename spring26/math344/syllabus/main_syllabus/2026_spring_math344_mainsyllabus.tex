...
\newcommand{\coursenumber}{MATH 344}
% Course Title
\newcommand{\coursetitle}{Applied Linear Algebra}
% Section
\newcommand{\coursesection}{}
% Term
\newcommand{\semester}{Sections 1 -- 2 --- Spring}
% Year
\newcommand{\myyear}{2026}
% Class Dates
\newcommand{\classdates}{January 12 -- May 6}
% Class Time
\newcommand{\classtimes}{MWF 9:40am -- 10:30am (Section 001); MWF 12:00pm -- 12:50pm (Section 002)}
% Classroom
\newcommand{\classroom}{Wardlaw 126 (Section 001); Close-Hipp 335 (Section 002)}
% Credits
\newcommand{\credits}{3}
% Instructor
...
\newcommand{\officehours}{MWF 2:30pm -- 3:30pm; TR 12:00pm -- 1:00pm}
...
\hspace{0.3cm} Syllabus Policy \dotfill \pageref{syllabus} 
...
% Course Information
\vspace{0cm} \largeheader{0.5cm}{Basic Course Information}{course_info} \vspace{-0.3cm}
...
\textit{Final Exam:} Friday, May 1, 9:00am (Section 001); Wednesday, April 29, 12:30pm (Section 002)
\sectionbreak
...
A student who successfully completes Applied Linear Algebra (MATH 344) should be able to\dots
	\begin{itemize} \itemsep=0.3ex
	\item Understand the concept of vectors and perform various computations with vectors. Additionally, students should understand some of the applications of vectors.
	\item Understand the concept of matrices and perform various computations with matrices. Additionally, students should understand some of the applications of matrices.
	\item Understand the connection between matrices and linear systems of equations. Additionally, students should be able to solve systems of linear equations using various matrix methods.
	\item Be able to compute the (R)REF of a matrix and understand the information that it conveys.
	\item Be able to compute and use the LU decomposition of a matrix.
	\item Be able to compute determinants and understand the information that it conveys.
	\item Be able to compute and apply the concept of a matrix inverse. 
	\item Be able to `represent' and understand the matrix equation $A\mathbf{x}= \mathbf{b}$ from several perspectives. Additionally, students should be able to solve this system using several different approaches and understand the pros/cons of each. 
	\item Understand the concept of a vector subspace. In particular, students should understand the concept of null space and column space.
	\item Students should be able to compute null spaces and column spaces and interpret the information these convey in several different ways.
	\item Understand the concept of linear independence and its connection with concepts such as null space, column space, and dimensions.
	\item Understand the concept of dimension and be able to compute dimensions of various linear algebra objects. 
	\item Understand the concept of eigenvalues and eigenvectors. In addition, students should be able to compute eigenvalues/vectors and be able to use them in various applications.
	\item Understand the concept of diagonalization and determine whether a matrix is diagonalizable. If so, students should be able to compute the diagonalization. Students should also know the applications of diagonalization. 
	\item Understand the concept and applications of orthogonalization. Students should also be able to perform various computations related to orthogonalization, e.g. Gram-Schmidt. 
	\item Know other linear algebra concepts and their various applications of linear algebra to the real world, e.g. SVD and PCP.
	\end{itemize}
...
{\itshape\bfseries\color{scred}Textbook.} The textbook for this course is {\itshape Introduction to Linear Algebra (6th Ed)} by Gilbert Strang (ISBN: 978-17331466-7-8). Students may purchase a physical or digital copy of the textbook; however, students are not required to a copy of the textbook. While references may be made to the text, the textbook will not be required to follow. Moreover, many free textbooks that can be used during the course will be linked on the course Blackboard. \pspace
...
{\itshape\bfseries\color{scred}Python \& Jupyter.} To help students engage with the course topics and learn some fundamental programming principles, students may be required to use Python through Jupyter Notebooks during the semester---during lecture, outside of class, or for homeworks. Python is a dynamic, interpreted programming language that is widely used in industry. Jupyter Notebook is an open-source application allowing one to easily create and group various coding language outputs. Lab computers will have Python and Jupyter easily accessible. Students will also be given instructions how to setup Python and Jupyter on their own computers. Both are freely available. If assigned, instructions will also be given on how to create and submit Python assignments to Blackboard as PDFs. \pspace

{\itshape\bfseries\color{scred}Calculators.} Basic graphing calculators will not be allowed during the course, unless otherwise instructed. Furthermore, calculators will generally not be allowed during the course. There may be times, however, when basic four-function calculators will be allowed. Students will be explicitly told when these are allowed. However, they will not be required. The course may make use of the computational engine Mathematica via the WolframAlpha website: \url{https://www.wolframalpha.com}. Although WolframAlpha does have a paid account option for additional resources, the course will not make use of these features and students {\itshape will not} be required to setup an account or make any kind of payment. The course may also make use of Symbolab: \url{https://www.symbolab.com/}. Calculators or other computational devices will not be allowed during exams unless otherwise instructed. 
\sectionbreak
...
The course consists of three meetings per week on Monday, Wednesday, and Friday. Lectures will begin with a short checkin followed by course material. Whenever possible, these lectures will consist of a topic discussion followed by time for individual or group problem solving. However, due to the number and depth of course topics, not every concept or problem type can be covered during class. Therefore, you may be assigned reading or videos before lecture. Lectures where readings or video are assigned will still cover course content; however, the focus of these lectures will then typically be problem-solving. Therefore, be sure to do the assigned reading/viewing before the lecture. Regardless, students are expected to spend outside of class reading course material, studying extra materials, and solving additional problems. Students are highly encouraged to do additional practice problems using the course resources provided in Blackboard. Finally, students are expected to typically spend approximately 3--4 hours per credit outside of class on course materials. However, some weeks this may be more or less.
\sectionbreak
...
You are encouraged to work with others on homeworks. Mathematics is a social activity! The purpose of working together on assignments is to engage with course topics, see different perspectives, ask questions, and have others look over your work. However, do not simply use others to do your assignments. You should also not allow other students to use you to complete their assignments. Of course, using online solutions is a violation of the university's academic integrity policies. Furthermore, while AI can help students in solving homework problems, you should only use an AI system as a tutor and not as a cheating system. That is, if a student uses an AI, they should try to use the AI to help them understand steps, concepts, etc. and not have it solve problems for them. Students should exercise \textit{extreme} caution using AI. Students that heavily rely on AI tend to have trouble memorizing required materials and recalling/understanding course concepts. Worse yet, students lose the experience in `struggling' through problems. Some struggle is natural in learning Linear Algebra---even important. This struggle is how students develop computation skills, learn problem solving skills, and build patience in problem solving---all extremely important skills for success in Mathematics. Using AI takes away the opportunity to learn from these experiences and build these mental `muscles.' The instructor recommends avoiding AI use as much as possible for the best chance of success in the course. Using an AI system to complete entire problems or homework, lab, etc. sets will be considered an academic integrity violation. If you are unsure of whether a particular resource is appropriate to use on an assignment, consult with your instructor first. \pspace
...
\mysection{0.24}{Important Dates}{univ_dates}
	...
	\begin{itemize}
	\item 01/19: MLK Day (No Classes)
	\item 01/20: Add/Drop Deadline
	\item 03/05: Midterm
	\item 03/07--03/15: Spring Break (No Classes)
	\item 03/30: Withdraw Deadline
	\item 04/27: Last Day of Classes
	\item 04/29--05/06: Final Exams (Including Saturday)
	\end{itemize}
...
The following is a \emph{tentative} schedule for the course and is subject to change. 
        ...
        02/20 & $Ax= b$ & 04/15 & Orthogonality \\
        02/23 & Rank & 04/17 & Orthogonality \\
