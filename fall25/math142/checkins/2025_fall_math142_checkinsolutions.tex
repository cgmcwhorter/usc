...
But then $\tan \theta= \tfrac{x}{3}$, which implies $x= 3 \tan \theta$. While this \text{seems} like it makes the statement of the problem false, this is not the only right triangle we could have constructed. If we had instead drawn
	...
	\end{tikzpicture}
	\]
We would have $\tan \theta= \tfrac{3}{x}$, which implies that $x \tan \theta= 3$, so that $x= \tfrac{3}{\tan \theta}= 3 \cot \theta$. This is the substitution in the problem statement. Both the substitutions $x= 3 \cot \theta$ and $x= 3 \tan \theta$ are viable trig. substitutions to compute this integral. 
