...
% 02/20
\checkin{02/20} Considering the infinite series $\ds\sum_{n=1}^\infty \dfrac{n - 1}{n^3}$ using the Divergence Test, because $\ds\lim_{n \to \infty} \dfrac{n - 1}{n^3}= 0$, the series converges converges.


% 02/25
\checkin{02/25} The series $\ds\sum_{n=1}^\infty 5 \left( \dfrac{2^{2n+1}}{3^n} \right)$ diverges. 
